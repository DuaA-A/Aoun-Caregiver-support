...
import { mentalExercisesData } from '../../data/mentalExercisesData';
import '../../styles/about.css';

const MentalExercises = () => {
  const { t, i18n } = useTranslation();
  const isRTL = i18n.language === 'ar';
  const data = mentalExercisesData[i18n.language] || mentalExercisesData['en'];
    ...
    <div className="about-alzheimer-page container" dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="about-header-wrapper animate-fade-in">
        <div style={{ maxWidth: '750px', margin: '0 auto', textAlign: 'center', position: 'relative', zIndex: 10 }}>
            <span className="about-subtitle">{t('mentalExercises.subtitle')}</span>
            <h1 className="about-title">{t('mentalExercises.title')}</h1>
            <p className="about-desc">{t('mentalExercises.desc')}</p>
        </div>
      </div>
...
export default MentalExercises;
